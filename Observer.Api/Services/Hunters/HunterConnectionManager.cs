...
public sealed class HunterConnectionManager
{
    ...
    private async Task<HunterConn> GetOrCreateAsync(string hunterId)
    {
        ...
        try
        {
            conn = await lazy.Value;
        }
        catch
        {
            // CreateAsync faulted — evict so the next call gets a fresh attempt
            _conns.TryRemove(new KeyValuePair<string, Lazy<Task<HunterConn>>>(hunterId, lazy));
            throw;
        }

        // WithAutomaticReconnect exhausts retries and leaves State = Disconnected.
        // Evict, dispose, and throw so the next PingAsync starts from scratch.
        if (conn.Bridge.State == HubConnectionState.Disconnected)
        {
            _conns.TryRemove(new KeyValuePair<string, Lazy<Task<HunterConn>>>(hunterId, lazy));
            try { await conn.Bridge.DisposeAsync(); } catch { /* ignore */ }
            throw new InvalidOperationException($"SignalR connection to Hunter '{hunterId}' is disconnected.");
        }
